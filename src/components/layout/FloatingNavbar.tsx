...
export function FloatingNavbar() {
  ...
  useEffect(() => {
    const index = itemsMenu.findIndex((i) => i.id === active);
  ...
  return (
    ...
    >
      <div
        ref={containerRef}
        className={cn(
          "relative flex items-center bg-[#F8FAFC]/80 backdrop-blur-xl md:rounded-full rounded-3xl transition-all duration-300 overflow-hidden text-black",
          isMobile
            ? isOpen
      ...
      >
        {(!isMobile || isOpen) && (
          <div
            className="absolute rounded-full bg-[#8EB8FD] transition-all duration-300 ease-out text-[#1E293B]"
        ...
        {isMobile && !isOpen && (
          <button
            ...
            className="flex items-center gap-2 text-text-[#1E293B] text-base font-medium px-3 py-2"
          >
            <span>{itemsMenu.find((i) => i.id === active)?.label}</span>
            <Menu size={18} />
          </button>
        )}
